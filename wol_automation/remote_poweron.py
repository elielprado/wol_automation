...
def wake_on_lan(mac_address):
    ...
    print(f'Número MAC: {mac_address}')
    mac_address = mac_address.upper()
    print(f'Número MAC Upper: {mac_address}')

    # Verifica se o endereço MAC tem o formato correto
    if len(mac_address) != MAC_LENGTH:
        print(f'Formato inválido: {len(mac_address)} caracteres')
        raise ValueError(
    ...
    print(f'Pacote Wake-on-LAN enviado com sucesso para {mac_address}')
...
def wake_on_lan_by_name(computer_name):
    ...
    try:
        print(
            f"Enviando Wake-on-LAN para {target_computer['name']} "
            f"({target_computer['hostname']})..."
        )
...
def wake_on_lan_all_auto():
    ...
    for comp in computers:
        if comp.get("auto_power_on", False):
            try:
                print(f"Enviando Wake-on-LAN para {comp['name']} ({comp['hostname']})...")
                wake_on_lan(comp["mac"])
                success_count += 1
            except Exception as e:  # pylint-disable=broad-except
                print(f"Erro ao enviar Wake-on-LAN para {comp['name']}: {e}")
...
def wake_on_lan_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                ...
                if 0 <= idx < len(computers):
                    try:
                        ...
                        print(f"Comando Wake-on-LAN enviado para {computers[idx]['name']}.")
                    except Exception as e:  # pylint-disable=broad-except
                        print(f"Erro ao enviar Wake-on-LAN: {e}")
                else:
            ...
            print(f"\n{success_count} de {len(computers)} computadores foram ligados com sucesso.")
